...
public class ControllerMainWindow implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        days = getAllDays("test", "SELECT * FROM test.temperatures");
        table.getItems().addAll(days);
    }

    public ArrayList<WeatherDay> getAllDays(String nameDB, String sqlQuery){
        ArrayList<WeatherDay> listDays = new ArrayList<>();
        try {
            Connection connection = WeatherDAO.getConnection(nameDB);
            ResultSet resultSet = WeatherDAO.query(connection, sqlQuery);
